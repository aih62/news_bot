...
def analyze_news_with_perplexity(news_list):
    ...
    **※ 주의: 한국 국내 뉴스나 정책은 선정에서 제외하고, 철저히 글로벌 동향 및 국제 규제 위주로 선정할 것.**
    ...
    - **중요: 각 <li> 항목은 반드시 중간에 마침표가 없는 '단 하나의 완벽한 문장'으로만 구성할 것.** 
    - 여러 개의 사실이나 인과관계를 설명할 때는 '~하며', '~하고', '~함에 따라' 등 맥락상 적합한 접속사와 연결 어미를 사용하여 문장이 끊기지 않고 자연스럽게 이어지도록 서술할 것.
    - **중요: 각 항목 시작 부분에 [배경], [핵심 내용] 등의 머리말(Label)을 절대 사용하지 말 것.**
    - 아래 4단계 스토리 구조를 각 항목에 자연스럽게 배분하여 서술할 것:
      1) (배경 및 지정학적 맥락) 지금 이 시점에 해당 뉴스 기사의 내용이 갖는 본질적인 의미가 무엇인지 상세히 설명할 것.
      2) (핵심 내용 및 메커니즘) 뉴스 기사 내용의 세부 사항을 비전문가인 정책 결정자도 직관적으로 이해할 수 있도록 쉽게 요약하되, 2분 이내에 속독이 가능하도록 핵심 위주로 풍부하게 서술할 것.
      3) (글로벌 파급력 및 산업 영향) 글로벌 공급망, 보안기업의 경쟁력, 혹은 국가 간 기술 표준 주도권과 주요국의 정책 및 제도 등에 미치는 실질적인 영향을 다각도로 분석할 것.
      4) (전망 및 한국 정책 시사점) 향후 전개될 구체적인 시나리오를 제시하고, 한국 정부의 보안 정책(예: 제로 트러스트 도입, 공급망 보안 입법 등)에 즉각 반영해야 할 전략적이고 구체적인 제언을 포함할 것.
