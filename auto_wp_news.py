...
def analyze_news_with_perplexity(news_list):
    ...
    [선정 기준]
    1. 심각성: 보안 사고나 취약점의 기술적 위험 수준이 얼마나 높은가
    2. 시급성: 즉각적인 대응이나 패치가 필요한 긴급한 사안인가
    3. 영향력: 대규모 인프라, 기업, 혹은 국가 단위의 광범위한 파급력이 있는가
    4. 정책성: 새로운 법규, 규제, 정부 정책 등 제도적 변화를 담고 있는가
    5. 미래가치: 향후 보안 패러다임 변화나 기술 발전에 중요한 시사점을 주는가
    ...
    - [심층 분석 리포트]: 
      ...
      * **각 <li> 항목(문장)은 100자 내외**로 작성하되, **문장 간의 논리적 연결(스토리텔링)**을 최우선으로 할 것.
      * '따라서', '또한', '결과적으로', '이러한 배경에서' 등 **적절한 접속어와 지시어**를 사용하여 전체 리포트가 하나의 자연스러운 이야기처럼 읽히게 작성할 것.
      * 사건의 배경 -> 기술적 메커니즘 -> 사회/경제적 파급력 -> 향후 전망 순의 **기승전결 구조**를 갖출 것.
      * **각 <li> 항목 내부에서 마침표(.)는 문장 끝에 단 한 번만 나오도록** 구성할 것.
      * 비전문가도 기술적 메커니즘을 쉽게 이해할 수 있도록 논리적으로 상세히 풀어서 작성할 것.
    - [전문가 코멘트]: <blockquote> 태그 사용. 기사의 시사점과 단기/중장기 대응 전략을 전문가적 시각으로 **150자 내외**로 정리하며, 마침표는 문장 끝에만 사용할 것.
