...
- [핵심 내용 요약]: <ul><li> 구조 사용.
  ...
  - [정보 밀도 — 필수]: 각 <li>는 반드시 180~250자로 작성(최소 150자, 최대 260자 — 260자를 넘기지 말 것). 한두 문장으로 빈약하게 끝내지 말되, 불필요한 만연체로 늘리지도 말 것. 문장 내 고유명사(기업·기술명·표준·기관 등)를 2개 이상 포함할 것.
...
[심층성 필수 지침]
- 웹 검색으로 확인한 사실에 더해, 당신의 보안 도메인 전문지식(공격 기법, 위협 그룹, 제품·표준, 규제 동향)을 활용하여 각 항목을 구체적으로 서술하십시오.
- 검색된 정보가 부족하더라도 제목과 도메인 지식을 바탕으로 배경·메커니즘·파급효과를 채워, 절대 빈약한 한두 문장으로 끝내지 마십시오. 단, 각 불릿은 260자를 넘기지 말고 핵심만 밀도 있게 담으십시오.
- 각 불릿에는 구체적 고유명사(기업/제품/기관/표준)와 가능한 한 정량 데이터를 담으십시오.
