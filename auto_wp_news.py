...
def analyze_news_with_perplexity(news_list):
    ...
    **※ 주의: 한국 국내 뉴스나 정책은 선정에서 제외하고, 철저히 글로벌 동향 및 국제 규제 위주로 선정할 것.**
    ...
    - 전체 분량: 반드시 4~5개의 <li> 항목으로 구성할 것.
    - **중요: 각 항목 시작 부분에 [배경], [핵심 내용] 등의 머리말(Label)을 절대 사용하지 말 것.**
    - 각 <li> 항목은 **최소 120자 이상의 완성된 문장**으로 작성할 것.
    - 아래 4단계 스토리 구조를 문장 속에 자연스럽게 녹여내어 작성할 것:
      1) (배경) 해당 사건이나 기술이 등장하게 된 지정학적·산업적 맥락을 상세히 서술할 것.
      2) (핵심 내용) 기술적 메커니즘이나 정책의 주요 골자를 비전문가도 이해할 수 있도록 쉽게 풀어 쓸 것.
      3) (파급력) 이 사안이 글로벌 경제, 외교, 혹은 산업 공급망에 미치는 실질적인 영향을 분석할 것.
      4) (전망 및 시사점) 향후 전개 방향과 함께 한국 정책 담당자가 주목해야 할 전략적 포인트를 제시할 것.
    - 모든 문장은 **'~다', '~하다'와 같은 서술형 어미**로 끝맺음할 것.
    - '이에 따라', '결과적으로', '특히' 등 접속어를 적극 활용하여 전체 리포트가 하나의 긴밀한 보고서처럼 읽히게 할 것.
