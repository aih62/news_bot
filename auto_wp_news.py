...
def analyze_news_with_perplexity(news_list):
    ...
    아래 제공된 뉴스 리스트 중 다음 기준에 따라 상위 10개 뉴스를 선정해,
    한국 정부 정책 담당자가 빠르게 동향을 파악할 수 있는 인텔리전스 리포트를 작성해줘.

    [선정 기준]
    1. 심각성: 보안 사고나 취약점의 기술적 위험 수준
    2. 시급성: 즉각적인 대응이나 패치가 필요한 긴급성
    3. 영향력: 대규모 인프라, 기업, 국가 단위의 파급력
    4. 정책성: 법규, 규제, 정부 정책 등 제도적 변화 여부
    5. 미래가치: 향후 보안 패러다임 변화에 대한 시사점
    ...
    - 전체 분량: 4~5개의 <li> 항목으로 구성.
    - 각 <li>는 1~2문장으로 간결하게 작성.
    - 아래 4단계 스토리 구조를 반드시 따르되, 각 단계를 1개 항목씩 배분할 것:
      1) [배경] 이 사건·정책·기술이 왜 지금 등장했는가 — 지정학적·산업적·제도적 맥락
      2) [핵심 내용] 무엇이 어떻게 작동하는가
         - 보안 사고라면: 어떤 방식으로 이루어졌는가
         - 정책·규제라면: 주요 조항과 도입 배경은 무엇인가
         - 기술 트렌드라면: 어떤 원리이며 기존과 무엇이 다른가
         → 비전문가도 이해할 수 있도록 쉬운 표현을 활용할 것.
      3) [파급력] 기업·정부·사회에 어떤 영향을 미치는가 — 경제·외교·산업적 파장
      4) [전망 및 시사점] 앞으로 어떻게 전개될 것이며, 한국 정책 담당자 입장에서 무엇을 주목해야 하는가
    - 항목 간 흐름이 자연스럽게 이어지도록 '이에 따라', '결과적으로', '한편' 등 접속어를 적절히 활용할 것.
    - 서술형(~했다, ~됐다, ~필요하다)으로 작성할 것.
    - 출처 번호([1], [web:1] 등)는 절대 포함하지 말 것.
    - 각 <li> 항목의 마지막 문장 끝에만 마침표를 사용할 것.
